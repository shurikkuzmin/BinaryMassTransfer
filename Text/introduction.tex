...
is often done in literature \cite{vanbaten-circular,kreutzer-overview}.  When the vortex is present
...
reactors and trickle bed reactors \cite{kreutzer-overview,bercic-mass}.  These reactors usually operate in
 the Bretherton-Taylor regime \cite{bretherton,taylor} which describes a flow through
...
comparatively small gas flow velocities \cite{yue-mass}. Due to the dominance of surface tension, the bubble train flow exhibits advantageous
 properties which can not be achieved
...
Thus, the understanding of mass transfer for the bubble train flow is not possible without deep understanding of hydrodynamic patterns.
There are several works studying the hydrodynamic properties of the bubble train flow: 
...
Since mass transfer coefficient is defined in terms of a mass flux from a certain area, Eq. \ref{eq:main:definition},
...
estimate the time for a fluid batch to travel whole circumference.   However,
...
hydrodynamics fields and complicated bubble shapes are taken into account.
...
with one bubble in it is used for simulations. Periodic boundary conditions are
...
coefficient, Eq. \ref{eq:main:definition} is obvious. With small
differences in results it can be either averaged in the liquid slug
concentration or the outlet space-averaged concentration as used in the formulation
of \citet{vanbaten-circular}. 

While it clear that periodic boundary conditions can be employed for the calculation
...
using the periodic boundary conditions with the same concentration at the inlet
...
The case we want to examine is a two-dimensionalbubble train flow between parallel plates.
...
\end{description}
In addition, at the end of the manuscript we present a comparison of dependence the volumetric
...
lenghts, the capillary number is the goal of future work.
\begin{figure}[htb!]
...
guidance as how to obtain the mass transfer coefficient for non-homogenous cases.
...
situation is much more complicated because of the presence of two phases and complicated bubble
...
the account the complicated mixing patterns and geometries. However, there are challenges as to how to
...
Thus, mass transfer simulations for a number of unit cells is needed. As it was
indicated above there are two approaches towards it - either to simulate the
bubble train and then to measure concentration along the pipe, Eq.
...
\Delta C}, 
\feq
where $\Delta C=\cstar-\langle C(t) \rangle$ with $\langle C(t) \rangle$ to be
the average concentration in whole liquid domain, $\dot{m}$ is the mass gain in
...
to the mass domain change minus whatever left domain plus whatever entered
domain. 
...
\feqstar
Then the mass change in the domain is only because of the generating bubble
surface. However, to calculate the mass transfer coefficient one needs to have
characteristic concentration  $\langle C(t) \rangle$ and mass gain $\dot{m}$
which do require the storage of time and space information.

Another limiting case (will be shown later) is hydrodynamic pattern change
...
\section{Validation}
As it was discussed analytical correlations for the mass transfer coefficient

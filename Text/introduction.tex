...
\maketitle
\begin{abstract}
This work gives the thorough procedure of how to determine the volumetric mass transfer coefficient
...
The volumetric mass transfer coefficient correlations are represented as a function of the diffusion
coefficient, slug and bubble lengths, and bubble velocity. For example, \citet{yue-mass} established
experimental correlation for periodic bubble train as: 
\begin{equation}
...
\cite{yue-mass}, the numerical validations to the best author knowledge are sparse. One of the
...
in different places. There are some works as how to avoid this approach and perform numerical
simulations for one unit cell. For example, \citet{vanbaten-circular} perform numerical simulations
for one unit cell using periodic boundary conditions and measure the volumetric mass transfer
coefficient as:
\begin{equation}
\label{main:simulation:equation}
k_L a=\frac{\text{Flux}}{\cbubble-C_{outlet}} \frac{\text{bubble surface area}}{\text{unit cell
volume}},
\end{equation}
...
at time
$t_1$ and time $t_2$ divided on the time difference $t_2-t_1$. The agreement between numerical
simulations and correlations of \citet{bercic-mass} was good. However, the numerical model only
accounts for circular capillaries and short contact times (the mass t), which indicate that the
bubble film is not saturated with the mass concentration. As well, authors took the approximation of
bubble shapes for small capillary numbers $Ca<0.01$ and then derived the hydrodynamics fields by
imposing a free-slip condition on the bubble surface. 

This work aims at the establishing numerical procedures as to how properly obtain the volumetric
mass transfer coefficient in the interesting range of capillary numbers, i.e. $0.1\leq Ca\leq1.0$.
The comparison of volumetric mass transfer coefficient correlations with experimental correlations
and establishing the Sherwood number as a function of other non-dimensional parameters is the goal
of the future work.

We chose this type of the problem due to different reasons:
\begin{description}
\item[I] The gas holdup, i.e. the ratio of volume occupied by bubble to unit cell volume, is
significant. Therefore, one cannot assume that the mass source is a point source or one can have
homogenous picture. Therefore, to establish the volumetric mass transfer coefficient one needs to
mimic the continuos picture. One of the goals of this work is to establish certain criteria as how
to simulate it numerically.
...
numbers there is no vortex and stremalines go around the bubble. Thus, the problem is interesting
because of really different hydrodynamic patterns depending on capillary number. Note that the flow
...
this work is to study mass transfer dependence on the number of parameters. 
\end{description}
 
The approach we want to use is the lattice Boltzmann method, a relatively new CFD
competitor emerged during last $20$ years \cite{frisch,mcnamara,HJ,HSB}. During years the method
was applied to simulate not only hydrodynamic problems \cite{yu}, but as well multiphase flows
\cite{Shan-chen:extended,swift,gunstensen}, heat transfer
...
framework. The manuscripts do not establish of how to obtain the volumetric mass transfer
coefficients. The work of \citet{inamuro-scalar-boundary} simulates heat and mass transfers in the
porous media. The work of \citet{jos-mass} simulates lateral mixing in cross-channel flow. While
two last works are focused at the mass transfer problems, both problems are of homogenuous nature
and do not guide as to how to simulate the non-homogenous problems.            

The overall goal of this paper is to address the questions arised and to establish the numerical
procedure suitable to measure the volumetric mass transfer coefficient. We will utilize the results
of numerical simulations of flow
obtained
before \cite{kuzmin-binary2d}. 

The paper is organized as follows. The mass transfer coefficients and volumetric mass transfer
coefficients are covered. The limiting cases from the continuos representation and their
application to the present simulation are given. The mass transfer simulations benchmarks are
presented. Finally, numerical simulations of hypotheses presented in this work and comparison
...
from the bubble. However, in the case bubble, the medium concentration can be the average
concentration in the domain, the flux averaged concentration at the outlet. Thus, one needs to
establish a thorough definition of the volumetric mass transfer coefficient in the case of complex
geometries and hydrodynamic patterns.
...
\Bigl(\frac{\lbubble}{\lbubble+\lslug}\Bigr)^0.3\\
&\vol \frac{\lunit}{\ugas+\uliq}=2\frac{\lunit}{d_h} \Bigl(\frac{D \varepsilon
}{(\ububble+\ugas)\lbubble+\lslug}\Bigr)^{0.5}
...
be average concentrations over different units. One can see Fig. \ref{fig:unit:8:scale:10} for

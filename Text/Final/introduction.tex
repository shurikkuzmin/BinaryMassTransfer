...
vortex in the slug ($Ca<0.7$) to not having it ($Ca>0.7$) \cite{giavedoni-numerical}.  In the latter case
...
in its macroscopic counterparts: thin liquid films \cite{bretherton} between bubbles and walls strongly enhance mass
...
hydrodynamic pattern in the slug (i.e. presence of a vortex in the slug), one can
...
with a square cross section. As well, in this work  a Schmidt number of around $0.8$ is  used:
thus, the thickness of the concentration and momentum boundary layer is similar
leading to qualitative results since it's not possible to separate mixing due to hydrodynamics patterns from mixing due to diffusion.} 

The usual simulation of mass transfer is
...
to  bubble train flow in order to derive expressions to connect space- and time-averages.  Then, the lattice
...
$\Bigl[\frac{kg}{m^3}\Bigr]$. Therefore, $k_L$ has units of 
...
\feq 
giving the same solution, now in the spatial domain:
\beq
...
In the application to bubble train flow it is useful to think of one bubble as a point
...
contributions from film and bubble caps can be calculated separately. Therefore, tracer from the film does not influence bubble caps diffusion.   However, this assumption overpredicts mass transfer for a number of experiments \cite{irandoust}. This happens since some tracer
...
concentrations as used in \cite{vanbaten-circular,kreutzer-overview}.  
...
\caption{The two-dimensional benchmark for the  the mass transfer
coefficient for the bubble located at the middle of the domain. \label{fig:benchmark}}
\end{figure}
...
\feqstar
In this case, Eq. \ref{main:main:main} reduces to Eq. \ref{eq:vanbaten:formulation}.
...
physical domain {\color{red} in non-dimensional terms} (hereafter we use subscript LB to denote non-dimensional quantities). It utilizes
...
\end{equation}
where $\bar{i}$ is the direction opposite to the $i$-th direction.
...
walls \cite{ginzburg-multireflection}, second-order accuracy of the
...
numbers governing the physics of the problem. In our case, this  is the Peclet number, $Pe=\frac{\ububble L}{D}$.
...
This point will be used in the simulations and covered later. 
...
{\color{red} As soon as we move to numerical simulations, all quantities are non-dimensional. Thus, we drop the subscript $LB$ for brevity.}
...
Time dependency goes via the non-dimensional time: $\tau=\frac{D t}{a^2}$. The domain size was $129\times 129$ with the circle radius $a=40$  lattice units. Some results for
...
which yields the Peclet number $Pe=U_0 \delta / (2 D)= 108.108$.
...
line with imposed free-slip boundary condition on it. The bubble velocity is then imposed on the walls. This corresponds to conducting 
...
within $3\%$ for all capillary numbers in the range $0.05\leq Ca \leq 1.0$. 
...
\item[Mass transfer] After the improved velocity profiles are obtained one can perform any mass
transfer simulation with the various boundary conditions as covered in Section \ref{section:cases}.
...
{\color{red} The simulations were performed using an in-house code with different modifications for different boundary conditions. A typical mass transfer simulation (domain size is $3000\times202$ cells) for $10^{6}$ iterations takes around $24$ hours on an Intel dual core CPU with the internal clock frequency of $2$ GHz. All simulations (serial for one unit cell and parallel for a few unit cells) are done using computers of the WestGRID high-performance clusters in Western Canada. }
...
\ref{fig:aver:conc:different:capillaries} for different capillary, Peclet numbers and velocity scalings
indicated in Table \ref{table:scaling:peclet}. When the average concentration gets
close to $\cstar=1$, Eq. \ref{theor:one:concentration:time} gives inadequate results due to the 
...
each simulation has a different physical time step. Thus, we normalized time such that it represents a number of unit cell lengths which the bubble has travelled, i.e. 
...
larger Peclet numbers fewer length is required to achieve the steady state condition. 
...
\caption{Volumetric mass transfer coefficient for different capillary numbers and velocity scales against the
bubble travel distance in the laboratory frame. The ''Cell Units`` axis refers to the physical
distance in unit cells lengths the bubble travels. A legend is provided for velocity scalings. All of them show a good agreement. One can see an abnormal rise of the mass transfer coefficient when the average concentration is close to $\cstar$ due to the logarithmic function evaluation.  Table
\ref{table:steady:state:average}
summarizes the results presented here.  \label{fig:aver:conc:different:capillaries}}
\end{figure}
\begin{table}[htb!]
...
Therefore the approach of \citet{vanbaten-circular} produces  accurate results if the characteristic concentration is the volume-averaged concentration (not the inlet/outlet flux-averaged concentration used in the original work). From the computational point of view, it also requires the concentration fields in time and space to calculate the mass change in time and the averaged domain
characteristic concentration.
...
simulations, see Fig. \ref{fig:moving:average:ca0097}, than with periodic boundary
simulations in Section \ref{main:results:periodic}. The characteristic concentration
...
to the head of a bubble train.  If end effects are eliminated then the average
...
One can see that the average concentrations reach a certain value and stay constant.
...
to use them in practical cases for unmixed slugs, i.e. $Ca>0.7$. \label{fig:periodic:ca1040}}
\end{figure}
...
correlation for the volumetric mass transfer coefficient was presented by
...
the correlation by \citet{yue-mass} is for three-dimensional cases. The fitting procedure based on the results of this work showed
that the power of
...
bubble.  We thoroughly performed and examined simulations with different

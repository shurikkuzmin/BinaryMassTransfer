...
void initialize_geometry()
{
	...
	//std::ifstream fux("vely0080000.dat");
	//std::ifstream fuy("velx0080000.dat");
	
	if ((fux==NULL) || (fuy==NULL))
	{
		std::cout<<"Get the fuck right name of the filename\n";
		exit(0);
	}
	...
	for(int counter=0;counter<NUM;counter++)
	{
		...
		fux>>uy[counter];
		fuy>>ux[counter];
		uy[counter]=coefficient*uy[counter];
		ux[counter]=coefficient*ux[counter];
	}

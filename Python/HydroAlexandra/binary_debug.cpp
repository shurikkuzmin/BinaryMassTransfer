   ...
   // {
	...
	for(int iX=0;iX<NX;iX++)
		for(int iY=0;iY<NY;iY++)
		{
			...
			for (int k=0; k<9; k++)
			{
				feq=weights[k]*dense_temp*(1.0+3.0*(cx[k]*ux_temp+cy[k]*uy_temp)
								+4.5*((cx[k]*cx[k]-1.0/3.0)*ux_temp*ux_temp+(cy[k]*cy[k]-1.0/3.0)*uy_temp*uy_temp+2.0*ux_temp*uy_temp*cx[k]*cy[k]));
				geq=weights[k]*phase_temp*(1.0+3.0*(cx[k]*ux_temp+cy[k]*uy_temp)
								+4.5*((cx[k]*cx[k]-1.0/3.0)*ux_temp*ux_temp+(cy[k]*cy[k]-1.0/3.0)*uy_temp*uy_temp
												 +2.0*ux_temp*uy_temp*cx[k]*cy[k]));
				f[iX][iY][k]=feq;
				g[iX][iY][k]=geq;
			}
...
void collide_bulk()
{
    ...
    for(int iX=0;iX<NX;iX++)
        for(int iY=1;iY<NY-1;iY++)
		{
            ...
            for (int k=0;k<9;k++)
            	force[k]=weights[k]*(3.0*force_x*cx[k]+3.0*force_y*cy[k]);

...
def Produce_Mass_Bunch_Sailfish_Average():
    #physical parameters
    gamma=0.0728
    diam=1.5e-3    
    print os.getcwd()
    ...
    for counter,dir_name in enumerate(dirs):    
        ...
        #coefficient=new_conc[:,1]/(200*3000)/(deltat*new_conc[:,0]*(1.0-numpy.abs(new_conc[:,2])))
        coefficient=new_conc[:,1]/(200*3000)/(deltat*new_conc[:,0]*(1.0-new_conc[:,1]/((1-gas_holdup)*200*3000)))        
        pylab.figure(99)
        pylab.plot(array[1:,0],coefficient,styles[counter])
        aver_coefficient.append(numpy.mean(coefficient[len(coefficient)/2:]))
    legs=[r'''$U_{\mathrm{bubble}}='''+str(vel)[:4]+r'''$''' for vel in bubble_velocities]
...
def Produce_Mass_Bunch_Grid():
    #physical parameters
    gamma=0.0728
    diam=1.5e-3    
    print os.getcwd()
 
    styles=['kv','ks','ko','k^','k>','k<','kD']
    aver_coefficient=[]
    bubble_velocities=[]
    #dirs=[3,5,8,10,20,40,60]
    dirs=[5,8,10,20,40,60]    
    fig=pylab.figure(99)
    ...
    for counter,dir_name in enumerate(dirs):    
        ...
        dims=2*numpy.array(dims)
        print "Dims=",dims
        dir_name="SailfishMassGrid/"+str(dir_name)+"/"
        #dir_name="SailfishMassJos/"+str(dir_name)+"/"     
        name=dir_name+"concentration.dat"
        array=numpy.loadtxt(name)        
        new_conc=numpy.array(zip(array[1:,0]-array[:-1,0],array[1:,1]-array[:-1,1],array[1:,2]))
        ...
        print "Data=",new_conc
        coefficient=new_conc[:,1]/((dims[0]-2)*dims[1])/(deltat*new_conc[:,0]*(1.0-numpy.abs(new_conc[:,2])))
        pylab.figure(99)
        pylab.plot(array[1:,0],coefficient,styles[counter])
        aver_coefficient.append(numpy.mean(coefficient[len(coefficient)/2:]))
    legs=[r'''$U_{\mathrm{bubble}}='''+str(vel)[:4]+r'''$''' for vel in bubble_velocities]
    ...
    pylab.ylabel(r'''$k_L a, \mathrm{s^{-1}}$''',fontsize=20)
    pylab.legend(legs,fancybox=True,labelspacing=0.1)
    pylab.savefig("steady_state_comparison_grid.eps",format="EPS",dpi=300)

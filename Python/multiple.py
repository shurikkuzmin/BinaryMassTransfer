...
def Produce_Triple():
    ...
    for counter,dir_name in enumerate(dirs):    
        ...
        results_dir_name="SailfishMassCentralTriple/"+str(dir_name)+"/"     
        ...
        for file_number in numpy.arange(1160000,4000000,280000):
            ...
            inlet_conc=numpy.sum(density[1:-1,1]*vx[1:-1,1])/numpy.sum(vx[1:-1,1])
            outlet_conc=numpy.sum(density[1:-1,-2]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])
            middle_1_conc=numpy.sum(density[1:-1,density.shape[1]/3]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])
            middle_2_conc=numpy.sum(density[1:-1,2*density.shape[1]/3]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])           
            print "Middle=",middle_1_conc,middle_2_conc
            
            coefficient_overall.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(3*15*diam)*numpy.log((1-numpy.abs(inlet_conc))/(1-numpy.abs(outlet_conc))))
            coefficient_first.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(inlet_conc))/(1-numpy.abs(middle_1_conc))))
            coefficient_second.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(middle_1_conc))/(1-numpy.abs(middle_2_conc))))
            coefficient_third.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(middle_2_conc))/(1-numpy.abs(outlet_conc))))
            
            
        coefficient_overall=numpy.array(coefficient_overall)
        coefficient_first=numpy.array(coefficient_first)
        coefficient_second=numpy.array(coefficient_second)
        coefficient_third=numpy.array(coefficient_third)
        print "Coefficient_first=",coefficient_first
        print "Coefficient_second=",coefficient_second
        print "Coefficient_third=",coefficient_third
        time_iterations=numpy.arange(1160000,4000000,280000)
        inds=numpy.where(time_iterations>3300000)        
        print coefficient_overall[inds[0]]        
        coeffs_overall.append(numpy.mean(coefficient_overall[inds[0]]))
        coeffs_first.append(numpy.mean(coefficient_first[inds[0]]))
        coeffs_second.append(numpy.mean(coefficient_second[inds[0]]))
        coeffs_third.append(numpy.mean(coefficient_third[inds[0]]))
        #numpy.savetxt(results_dir_name+"coefficient.dat",zip(time_iterations,coefficient))           
        ...
        pylab.plot(time_iterations,coefficient_second,styles[counter])
        pylab.figure(4)
        pylab.plot(time_iterations,coefficient_third,styles[counter])
        
        #aver_coefficient.append(numpy.mean(coefficient[len(coefficient)/2:]))
    print "Coeffs_overall=", coeffs_overall
    print "Coeffs_first=",coeffs_first
    print "Coeffs_second=",coeffs_second
    print "Coeffs_third=",coeffs_third
    file_names=["steady_triple_overall.eps","steady_triple_first.eps","steady_triple_second.eps","steady_triple_third.eps"]        
...
def Produce_Quadro():
    ...
    for counter,dir_name in enumerate(dirs):    
        ...
        results_dir_name="SailfishMassCentralQuadro/"+str(dir_name)+"/"     
        ...
        for file_number in numpy.arange(1160000,4000000,280000):
            ...
            inlet_conc=numpy.sum(density[1:-1,1]*vx[1:-1,1])/numpy.sum(vx[1:-1,1])
            outlet_conc=numpy.sum(density[1:-1,-2]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])
            middle_1_conc=numpy.sum(density[1:-1,density.shape[1]/4]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])
            middle_2_conc=numpy.sum(density[1:-1,2*density.shape[1]/4]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])           
            middle_3_conc=numpy.sum(density[1:-1,3*density.shape[1]/4]*vx[1:-1,-2])/numpy.sum(vx[1:-1,-2])            
            print "Middle=",middle_1_conc,middle_2_conc,middle_3_conc
            
            coefficient_overall.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(4*15*diam)*numpy.log((1-numpy.abs(inlet_conc))/(1-numpy.abs(outlet_conc))))
            coefficient_first.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(inlet_conc))/(1-numpy.abs(middle_1_conc))))
            coefficient_second.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(middle_1_conc))/(1-numpy.abs(middle_2_conc))))
            coefficient_third.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(middle_2_conc))/(1-numpy.abs(middle_3_conc))))                 
            coefficient_fourth.append((superficial_liquid+gas_holdup*interface_velocity)*phys_velocity/interface_velocity\
                   /(15*diam)*numpy.log((1-numpy.abs(middle_3_conc))/(1-numpy.abs(outlet_conc))))
        ...
        coefficient_second=numpy.array(coefficient_second)
        coefficient_third=numpy.array(coefficient_third)
        coefficient_fourth=numpy.array(coefficient_fourth)
        print "Coefficient_first=",coefficient_first
        print "Coefficient_second=",coefficient_second
        print "Coefficient_third=",coefficient_third
        print "Coefficient_fourth=",coefficient_fourth
        time_iterations=numpy.arange(1160000,4000000,280000)
        inds=numpy.where(time_iterations>3300000)        
        print coefficient_overall[inds[0]]        
        coeffs_overall.append(numpy.mean(coefficient_overall[inds[0]]))
        ...
        coeffs_second.append(numpy.mean(coefficient_second[inds[0]]))
        coeffs_third.append(numpy.mean(coefficient_third[inds[0]]))
        coeffs_fourth.append(numpy.mean(coefficient_fourth[inds[0]]))
        #numpy.savetxt(results_dir_name+"coefficient.dat",zip(time_iterations,coefficient))           
        ...
        pylab.plot(time_iterations,coefficient_third,styles[counter])
        pylab.figure(5)
        pylab.plot(time_iterations,coefficient_fourth,styles[counter])
    ...
    print "Coeffs_second=",coeffs_second
    print "Coeffs_third=",coeffs_third
    print "Coeffs_fourth=",coeffs_fourth
    file_names=["steady_quadro_overall.eps","steady_quadro_first.eps","steady_quadro_second.eps","steady_quadro_third.eps","steady_quadro_fourth.eps"]        

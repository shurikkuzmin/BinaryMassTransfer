...
void collide_bulk()
{
    ...
    for(int iX=0;iX<NX;iX++)
        for(int iY=1;iY<NY-1;iY++)
		{
			...
			double vx = ux[iX][iY] / rho[iX][iY];
			double vy = uy[iX][iY] / rho[iX][iY];

			ux[iX][iY]=ux[iX][iY]/rho[iX][iY]+0.5*force_x;
			uy[iX][iY]=uy[iX][iY]/rho[iX][iY]+0.5*force_y;
			...
			for (int k=1; k<9; k++)
			{
				feqeq[k]=weights[k]*(3.0*pressure_bulk+3.0*dense_temp*(cx[k]*vx+cy[k]*vy)
								+4.5*dense_temp*((cx[k]*cx[k]-1.0/3.0)*vx*vx+(cy[k]*cy[k]-1.0/3.0)*vy*vy+2.0*vx*vy*cx[k]*cy[k]))
				+kconst*(wxx[k]*gradx_temp*gradx_temp+wyy[k]*grady_temp*grady_temp+wxy[k]*gradx_temp*grady_temp);
            ...
            {
				// *(1.0-0.5*omega_rho)
				force[k]=weights[k]*dense_temp*(3.0*force_x*cx[k]+3.0*force_y*cy[k]);
            }
...
int main(int argc, char* argv[])
{
	...
	for(int counter=0;counter<=N;counter++)
	{
		...
		//Writing files
		if (counter <= 10 || (counter < 100 && counter % 100 == 0) || (counter % 1000 == 0))
		{
